...
def evaluate_model(model, x_test, y_test, name, output_dir="data/models"):
    """
    Evaluate a trained model and save performance outputs to disk.

    Args:
        model: Trained scikit-learn compatible model.
        x_test (pd.DataFrame): Test feature matrix.
        y_test (pd.Series): True labels for the test set.
        name (str): Name of the model (used for saving files).
        output_dir (str): Directory to save model and confusion matrix.
...
def train_and_evaluate_all_models(train_df, use_smote=True, selected_models=None):
    """
    Train and evaluate classification models on the provided dataset.

    Supports optional SMOTE oversampling and selective model inclusion.

    Args:
        train_df (pd.DataFrame): DataFrame containing features and target.
        use_smote (bool): Whether to apply SMOTE oversampling.
        selected_models (list[str]): List of model identifiers to train (e.g., 'random_forest', 'xgboost').
